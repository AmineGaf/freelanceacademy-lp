      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 h-full w-full bg-black/40 backdrop-blur-[1px] z-[3]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          />
...
function DialogVideo({ code, className, style, horizontal }: DialogVideoProps) {
  const { uniqueId } = useDialog();
  ...
  return (
    <motion.div
      ...
      className={cn(
        className,
        "relative",
        horizontal ? "pt-[80.25%] max-sm:pt-[170.25%]" : " pt-[56.25%]"
      )}
    >
      <motion.iframe
        loading="lazy"
        title="Gumlet video player"
        src={`https://play.gumlet.io/embed/${code}`}
        style={style}
        className={cn(
          "border-none absolute top-0 left-0 h-full w-full transition-all",
      ...
      {!loadStatus && (
        <div className="top-1/2 left-1/2 absolute -translate-x-1/2 -translate-y-1/2">
          <div className="border-2 border-white-w1/50 border-b-[transparent] border-solid rounded-full w-6 h-6 animate-spin" />
        </div>
      )}
    </motion.div>
  );
