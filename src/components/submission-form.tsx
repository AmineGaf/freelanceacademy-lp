        ...
        >
            {formContent.items.map((element) => (
                <div className="flex flex-col gap-2" key={element.value}>
                    <label
                        ...
                        className="text-grey-w1 text-right font-sans font-light text-h5 transition-colors duration-200 hover:text-grey-w0"
                    >
                        {element.label}
                    </label>
                    <input
                    ...
                    {errors[element.value] && (
                        <p className="text-sm text-red-500">
                            {(errors[element.value]?.message as string) ||
                                `${element.label} مطلوب`}
                        </p>
                    )}
                </div>
            ...
            <div className="flex flex-col gap-4 items-start">
                <button
                    ...
                    className="px-6 py-3 rounded-lg bg-dark-blue text-grey-w0 text-sm font-medium hover:bg-blue-w3 transition-colors"
                >
                    {isSubmitting
                        ? "جاري الإرسال..."
